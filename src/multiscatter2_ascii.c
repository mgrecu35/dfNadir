...
int multiscatter2_(int *nrange, float *extFort, 
		   ...
		   float *bscatFort, int *noMS, 
                   float *alt, float *theta, float *freq, float *dr)
{
  ...
  instrument.wavelength=300.0/(*freq)/1000.0;
  instrument.altitude=(*alt)*1000.0;
  instrument.rho_receiver[0]=*theta/180.0*pi;
  //printf("%g \n",instrument.rho_receiver[0]);
  instrument.rho_receiver[0]=*theta/180.0*pi;
  instrument.rho_transmitter=*theta/180.0*pi;
