...
!------------------------------snow exp tables---------------------!
  ...
  real,allocatable :: z35TableS2(:,:), att35TableS2(:,:), z94TableS2(:,:), z94Table(:,:),  z94TableH(:,:), &
       pwc35TableS2(:,:)
...
subroutine readTablesLiang2(extnmu,extnmfreq)
  ...
  allocate( z35TableS(nbins,nmu), att35TableS(nbins,nmu), &
       pwc35TableS(nbins,nmu))
  allocate( z35TableS2(nbins,nmu), z94TableS2(nbins,nmu), &
       z94TableH(nbins,nmu),att35TableS2(nbins,nmu), &
...
!     open(10,file='nw_d0_dsd/snowgTables.rho01.mu2.2')
     ...
     do j=1,7
        ...
        do i=1,nbinH
           ...
           if (j==5) then
              z94TableH(i,imu)=33.206+&
                   10.0*log10(salbTableH(i,5,imu)*henyey(asymTableH(i,5,imu)))
           end if
        enddo
     enddo
