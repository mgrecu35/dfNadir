...
def readScatProf(fname):
    ...
    return temp,mass,fraction,bscat,Deq, ext, scat, g
...
def readScatProfR(fname):
    ...
    g=fh['scat'][:]
    Deq=10*(mass*1e3*6/pi)**(0.333) # in mm
    return temp,mass,bscat,Deq, ext, scat, g

tempKu,massKu,fractionKu,bscatKu,DeqKu,extKu,scatKu,gKu=readScatProf(fnameIceKu)
tempKu_r,massKu_r,bscatKu_r,DeqKu_r,extKu_r,scatKu_r,gKu_r=readScatProfR(fnameRainKu)

tempKa,massKa,fractionKa,bscatKa,DeqKa,extKa,scatKa,gKa=readScatProf(fnameIceKa)
tempKa_r,massKa_r,bscatKa_r,DeqKa_r,extKa_r,scatKa_r,gKa_r=readScatProfR(fnameRainKa)
...
def fmodels(sAtt_coeffs,rAtt_coeffs,swc_coeffs,rwc_coeffs,zKu,iprof,n4,dn,\
    ...
    zKaL_jl,kextKa_jl,scatKa_jl,\
        gKa_jl,zKaL_true,piaKa=jl.simZKa(bscatKa[-1,12,:],extKa[-1,12,:],DeqKa[12,:],\
                                         scatKa[-1,12,:],gKa[-1,12,:],\
                                         bscatKa_r[-9,:],extKa_r[-9,:],DeqKa_r,scatKa_r[-9,:],gKa_r[-9,:],\
                                         wlKa,dns,dnr,swc,rwc,dr,kextAtm35[n4[0]:n4[-1]])
    
    zKuL_jl,kextKu_jl,scatLKu_jl,\
        g_jl,zKuL_true,piaKu=jl.simZKa(bscatKu[-1,12,:],extKu[-1,12,:],DeqKu[12,:],\
                                       scatKu[-1,12,:],gKu[-1,12,:],\
                                       bscatKu_r[-9,:],extKu_r[-9,:],DeqKu_r,scatKu_r[-9,:],gKu_r[-9,:],\
                                       wlKu,dns,dnr,swc,rwc,dr,kextAtm35[n4[0]:n4[-1]])
