...
function get_fZs(zObs,ns,bscat,scat,ext,g,Deq,vfall,wl,dn,mu)
    rwc=0.1
    #println(ns)
    #println(wl)
    #println(dn)
            ...
            if(zTn>-10 && zTn<50)
                if i<=nmTop
                    ...
                    if i>=nmBot
                        zTn=max(0,zTn)
                        zTn=min(zTn,45)
                        zKu[i]=pzCoeffs[11,1]*zTn^2+pzCoeffs[11,2]*zTn+pzCoeffs[11,3]+10*log(dn1)
                        ...
                        zKu[i]=pzCoeffs[i0,1]*zTn^2+pzCoeffs[i0,2]*zTn+pzCoeffs[i0,3]+10*log(dn11)
